...
class CANExtractor:
    ...
    def process(self, raw_data, index):
        ...
        yaw_list = np.empty((len(raw_data), 110, 1), dtype=np.float32)
        ...
        input_yaw = torch.tensor(yaw_list[:,0:50,:], dtype=torch.float)
        
        input_dr_x = torch.tensor(dr_data_list[:,0:50,:], dtype=torch.float)
        input_dr_y = torch.tensor(dr_data_list[:,50:110,:], dtype=torch.float)
        x_ctrs = input_dr_x[:, 49,:]
        # y = torch.stack(input_dr_data, dim=-1)
        # print(y.shape)
        # padding_mask = np.empty((len(raw_data),60, 1), dtype=np.bool)
        ...
        x_heading = torch.zeros((len(raw_data), 50, 1), dtype=torch.float)
