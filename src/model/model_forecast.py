...
from .layers.multimodal_decoder_ver2 import MultimodalDecoder
...
class ModelForecast(nn.Module):
    def __init__(
        ...
        self.hist_embed = AgentEmbeddingLayer(
            9, embed_dim // 4, drop_path_rate=drop_path         # 채널 수 조정
        )
    ...
    def forward(self, data):
        # print(data["can_yaw_rate"].shape)
        # print(data["x_padding_mask"].shape)
        ...
        hist_feat = torch.cat(
            [
                data["x"],
                data["can_yaw_rate"],
                data["can_wheel_speed"],
                data["can_steering_spd"],
                data["can_steering_ang"],
                data["can_lateral_accel"],
                data["can_longitudinal_accel"],
        ...
        hist_feat = hist_feat.view(B, L, D)
        hist_feat_key_padding = hist_key_padding_mask.view(B)
        # print(hist_feat_key_padding.shape)
        # print(hist_feat[~hist_feat_key_padding].shape)
        actor_feat = self.hist_embed(
            hist_feat[~hist_feat_key_padding].permute(0, 2, 1).contiguous()
        )
        actor_feat_tmp = torch.zeros(
            B, actor_feat.shape[-1], device=actor_feat.device
        )
        actor_feat_tmp[~hist_feat_key_padding] = actor_feat
        actor_feat = actor_feat_tmp.view(B, actor_feat.shape[-1])
        ...
        x_angles = torch.cat([torch.cos(angles), torch.sin(angles)], dim=-1)        # 차원 조정 ([2,59,50,1,2]->[2,59,50,2])
        ...
        y_hat, pi = self.decoder(x_encoder)
